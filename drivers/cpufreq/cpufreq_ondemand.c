...
#define LIMIT_MAX_FREQ_BROWSING // heat problem for browsing
#endif

#define _LIMIT_LCD_OFF_CPU_MAX_FREQ_

bool lmf_inputmethod_state=false;
bool lmf_powersavemode_state=false;

bool lmf_touchbooster_enable = true;

void set_lmf_touchbooster(bool onOff);
bool get_lmf_touchbooster(void);

void set_lmf_powersavemode(bool onOff);
bool get_lmf_powersavemode(void);
void set_lmf_touchbooster_onoff(bool onOff);
bool get_lmf_touchbooster_onoff(void);
/*
...
#ifdef LIMIT_MAX_FREQ_BROWSING // limit max freq
...
static ssize_t show_lmf_touchbooster(struct kobject *kobj,
				      struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", get_lmf_touchbooster());
}

static ssize_t show_lmf_powersavemode(struct kobject *kobj,
				      struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", get_lmf_powersavemode());
}

static ssize_t show_lmf_touchbooster_onoff(struct kobject *kobj,
				      struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", get_lmf_touchbooster_onoff());
}
...
static ssize_t store_lmf_touchbooster(struct kobject *a, struct attribute *b,
				   const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	//mutex_lock(&dbs_mutex);
	set_lmf_touchbooster(input);
	//mutex_unlock(&dbs_mutex);

	return count;
}

static ssize_t store_lmf_powersavemode(struct kobject *a, struct attribute *b,
				   const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	//mutex_lock(&dbs_mutex);
	set_lmf_powersavemode(input);
	//mutex_unlock(&dbs_mutex);

	return count;
}

static ssize_t store_lmf_touchbooster_onoff(struct kobject *a,
		struct attribute *b, const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	//mutex_lock(&dbs_mutex);
	set_lmf_touchbooster_onoff(input);
	//mutex_unlock(&dbs_mutex);
...
#endif
define_one_global_rw(lmf_touchbooster);
define_one_global_rw(lmf_powersavemode);
define_one_global_rw(lmf_touchbooster_onoff);
...
#endif
	&lmf_touchbooster.attr,
	&lmf_powersavemode.attr,
	&lmf_touchbooster_onoff.attr,
	NULL
};
...
bool get_lmf_touchbooster(void)
{
	return lmf_inputmethod_state;
}

bool get_lmf_powersavemode(void)
{
	return lmf_powersavemode_state;
}

bool get_lmf_touchbooster_onoff(void)
{
	return lmf_touchbooster_enable;
}

void set_lmf_touchbooster(bool onOff)
{
	printk(KERN_ERR "change inputmethod %d \n",onOff);
	if(onOff)
	{
		lmf_inputmethod_state = true;
		printk(KERN_ERR "inputmethod cpu limits 1.0GHz \n");
		cpufreq_set_limit(INPUTEDITOR_START);
	}else
	{
		lmf_inputmethod_state = false;
		printk(KERN_ERR "inputmethod cpu limits 1.5GHz \n");
		cpufreq_set_limit(INPUTEDITOR_STOP);
	}
}

void set_lmf_powersavemode(bool onOff)
{
	printk(KERN_ERR "powersavemode %d \n",onOff);
	if(onOff)
	{
		lmf_powersavemode_state = true;
		printk(KERN_ERR "powersave cpu limits 800MHz \n");
		cpufreq_set_limit(POWERSAVING_START);
	}else
	{
		lmf_powersavemode_state = false;
		printk(KERN_ERR "powersave cpu limits 1.5GHz \n");
		cpufreq_set_limit(POWERSAVING_STOP);
	}
}
void set_lmf_touchbooster_onoff(bool onOff)
{
	pr_info("[%s] %s\n", __func__, onOff ? "on" : "off");
	lmf_touchbooster_enable = onOff ? true : false;
}
...
#define ACTIVE_DURATION_MSEC	(2*60*1000) // 2 mins (HW request)
#else
		...
		else
		{
			...
			else
			{
				...
				if (time_int >= SAMPLE_DURATION_MSEC)
				{
					...
					ave_max = (time_int / 10) * ((LMF_ACTIVE_MAX_FREQ/1000) * 2);
...
#endif
#if 0
	if (unlikely(!lmf_touchbooster_enable))
		return;

	pr_info("[%s] set to max\n", __func__);
#endif
