...
#endif
#else // CONFIG_FB_MSM_MDP_ADDITIONAL_BUS_SCALING
#ifdef CONFIG_MSM_BUS_SCALING
#define OVERLAY_BUS_SCALE_TABLE_BASE	6
#endif
#endif // CONFIG_FB_MSM_MDP_ADDITIONAL_BUS_SCALING
...
#ifdef CONFIG_FB_MSM_MDP_ADDITIONAL_BUS_SCALING 
	if(require_exceptional_MDP_clock) nLoop = 1;
#endif 

	if(nLoop == 0 && (mfd->panel_info.type == MIPI_VIDEO_PANEL))
	{		
...
#ifdef CONFIG_FB_MSM_MDP_ADDITIONAL_BUS_SCALING // Changed code - SS changed the code...
#define CCAMCORDING_ORIGINAL_PERF_LEVEL (3)
#ifdef CONFIG_MSM_BUS_SCALING
	if (pipe->mixer_num == MDP4_MIXER0) {
		if(perf_level == CCAMCORDING_ORIGINAL_PERF_LEVEL && require_exceptional_MDP_clock )
		{
			mdp_bus_scale_update_request(OVERLAY_BUS_SCALE_TABLE_BASE - 1);
			printk( "MDP_ADDITIONAL_BUS_SCALING : index=%d\n", OVERLAY_BUS_SCALE_TABLE_BASE - 1 );
		}
		else
		{
