...
)
{
	...
	*/
	...
	if (!dwrq->length && !g_call_join_disassoc) {
...
set_ap_cfg(struct net_device *dev, struct ap_profile *ap)
{
	...
	if (ap_cfg_running == FALSE) {
		...
		WL_TRACE(("\n>in %s: apsta set result: %d \n", __FUNCTION__, res));
               mpc = 0; /* XXX andrey: nail mpc to 0 once again here */
                 if ((res = dev_wlc_intvar_set(dev, "mpc", mpc))) {
                         WL_ERROR(("%s fail to set mpc\n", __FUNCTION__));
                         goto fail;
                 }
...
#if ((defined(CONFIG_USA_MODEL_SGH_I717) || defined(CONFIG_USA_MODEL_SGH_T769) || defined(CONFIG_USA_MODEL_SGH_I577) || defined(CONFIG_CAN_MODEL_SGH_I577R)) && defined(CHECK_CHIP_REV)) 
		if (g_chipver == 4) {
