					...
					char *buf)
{
	...
	return sprintf(buf, "%d\n", pwr->interval_timeout);
		...
		|| defined (CONFIG_KOR_MODEL_SHV_E120L) || defined (CONFIG_KOR_MODEL_SHV_E160S) \
		 || defined (CONFIG_KOR_MODEL_SHV_E160K) || defined(CONFIG_KOR_MODEL_SHV_E160L)
#define YAMATO_REAL_MAX_FREQ		266667000 // yamato max
static unsigned int yamato_freq_max = 0;
#endif
...
/*
		...
		|| defined (CONFIG_KOR_MODEL_SHV_E120L) || defined (CONFIG_KOR_MODEL_SHV_E160S) \
		 || defined (CONFIG_KOR_MODEL_SHV_E160K) || defined(CONFIG_KOR_MODEL_SHV_E160L)
*/		 
	if (yamato_freq_max == 0)
		yamato_freq_max = pwr->pwrlevels[0].gpu_freq;

	if (reset == 0)	
		pwr->pwrlevels[0].gpu_freq = YAMATO_REAL_MAX_FREQ;
	else
		pwr->pwrlevels[0].gpu_freq = yamato_freq_max;
#endif    
