...
static bool g_tdmb_power_on = false;
...
{
	DPRINTK("%s : g_tdmb_power_on(%d)\n", __func__, g_tdmb_power_on);
...
static ssize_t tdmb_read(struct file *file, char __user *buf, size_t nbytes, loff_t *ppos)    
{
	DPRINTK("tdmb_read!\n");
...
static TDMBDrvFunc * tdmb_get_drv_func(struct tdmb_platform_data * gpio)
{
	TDMBDrvFunc* (*func)(struct tdmb_platform_data * gpio);
	extern unsigned int get_hw_rev(void);

#if defined(CONFIG_TDMB_T3900) || defined(CONFIG_TDMB_T39F0)
	extern TDMBDrvFunc * T3900_drv_func(struct tdmb_platform_data * gpio);
#endif
#if defined(CONFIG_TDMB_FC8050)
	extern TDMBDrvFunc * FC8050_drv_func(struct tdmb_platform_data * gpio);
#endif
#if defined(CONFIG_TDMB_SIMUL)
	extern TDMBDrvFunc * TFF_drv_func(struct tdmb_platform_data * gpio);
#endif

#if defined(CONFIG_TDMB_SIMUL)
	func = TFF_drv_func;
#elif ( (defined(CONFIG_TDMB_T3900) || defined(CONFIG_TDMB_T39F0)) && defined(CONFIG_TDMB_FC8050) )
