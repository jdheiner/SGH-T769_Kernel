...
static int voice_set_device(struct voice_data *v)
{
	...
	/* Send start vpcm and wait for response. */
		...
		pr_info(" send vpcm_device_change_cmd, pkt_size=%d vol_index = %d  rx_device=%d  tx_device=%d\n",
...
static int voice_send_vol_index_to_modem(struct voice_data *v)
{
	...
	pr_info(" send vpcm_volume_change_cmd, pkt_size= %d vol_index = %d\n",
...
#if defined(CONFIG_KOR_MODEL_SHV_E120S) || defined(CONFIG_KOR_MODEL_SHV_E120K) || defined(CONFIG_KOR_MODEL_SHV_E120L) || defined(CONFIG_KOR_MODEL_SHV_E160L) || defined(CONFIG_USA_MODEL_SGH_T769) //kks_111020 // Qualcomm Gon's workaround code to solve the sound mute problem after subsystem reset(SSR) during voice call(QC case 645569)
// Added config CONFIG_USA_MODEL_SGH_T769 to resolve issue P111122-0890
		if( (v->voc_state == VOC_RUN) && (NULL == voice_get_apr_mvm(v))) 
		{
